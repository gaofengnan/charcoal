...
#' @description entries of v are moved towards 0 by the amount lambda until they hit 0. From github.com/wangtengyao/putils.
#' @export
...
#' @return a vector of the same length
#' @description entries of v that are below lambda are set to 0. From github.com/wangtengyao/putils.
#' @export
...
#' Computing the orthogonal matrix spanning the orthogonal complement of the range of X
...
#' Generate a sequential linear regression sample with a single changepoint or multiple changepoints
#' @param n the total number of data points
#' @param tau a number (in case of single changepoint) or vector containing all changepoints in fraction. If tau = c(0.1,0.2), the changes takes place at 0.1n and 0.2n
#' @param k a vector containing the sparsity of each vector of change, if k is of length 1, then the same sparsity is applied to all change vectors. E.g., k = c(3,5), the first change vector is 3-sparse and the second 5-sparse
#' @param rho signal strength at each changepoint, i.e. the l2 norm of each change vector, if rho is of length 1, then the signal strength is applied to all changes
#' @param design a string specifying the distribution of the random design, implemented options include "gaussian" for GOE design;"t4" for t distribution with degree of freedom 4 (4 can be replaced by any desired df, and if unspecified, df of 4 is applied); "correlated" or "ar" for each row generated by N_p(0,Sigma), where Sigma has (i,j) entry of 0.7^(i-j); "anova" for one-way anova design; and "rademacher" for all entries of X being independent fair coin toss between {-1,1}
#' @param error a string specifying the distribution of observational errors in linear regression, the implemented options include "gaussian" for standard normal errors;"t4" for t distribution with degree of freedom 4 (4 can be replaced by any desired df, and if unspecified, df of 4 is applied); "exp" the centered exponential distribution; "laplace" for standardized Laplace distribution with zero mean and std 1; and "rademacher" for all errors of being independent fair coin toss between {-1,1}
...
#' @return a list containing (X, Y, beta, tau). X as the design matrix and Y as the response vector, beta as the regression coefficient matrix with each column corresponding to each time point, tau as the fraction of n where true changes take place
#' @description Generate a sequential linear regression sample with multiple changepoints, with options for different distributions of both random design and observational error
#' @export
...
#' Internal function to generate random intervals for not_cpreg, the main multiple changepoint estimation function in linear regressions as in Algorithm 4
...
#' @param delta only intervals with length longer than (1+delta) * p will be kept
...
#' @param alpha the critical type-I error probablity threshold, should in practice adjusted for the multiple testing, i.e., in this case the number of intervals
#' @param sigma the standard deviation of the noise in the regressions
#' @param b the standard deviation of each entry of the randomly generated regression coefficients, which follows N(0,b^2)
#' @param verbose whether to output intermidate results/progress bar in the console
#' @return a nonnegative scalar as the threshold for not_cpreg
#' @description Generate the threshold for combining charcoal with the narrowest-over-threshold algorithm, which is also used as the critical threshold in the testing refining stage in not_cpreg.  It works by generating permSize Monte Carlo repetitions of running cpreg on the null model without any change in regression coefficients
#' @export
...
#' @param lambda threshold for Algorithms 1 and 2, set it to NULL and the recommended value is applied
...
#' @param burnIn specifies the fraction at both ends of the interval to be discarded as possible changes, to handle common boundary effects
#' @param verbose whether to output intermidate results/progress bar in the console
...
#' in Gao and Wang (2022), Algorithm 4
#' @param X design matrix of the linear regression model
#' @param Y response vector of the linear regression model
#' @param sigma noise standard deviation of the regression models (must be supplied)
#' @param no_intervals number of intervals for the narrowest-over-threshold part
#' @param seed the random seed for the ensuing random interval generation for the narrowest-over-threshold. The default is NULL, where no seed is set.
#' @param Thresh the threshold for the narrowest-over-threshold, and the critical value in the testing refinement stage
#' @param cpreg_method specifies which variant of the algorithms to be applied in the refinement stage. Set it to 'compsket' for Algorithm 1, to 'proj' for Algorithm 2, and to 'lasso_bic' for Algorithm 3. The default is 'lasso_bic'. The variant employed in the NOT stage is always 'proj' in the current implementation.
#' @param burnIn the burnIn parameter to be passed to cpreg in both the NOT , which specifies the fraction at both ends of the interval to be discarded as possbile changes, to handle common boundary effects.
#' @param verbose whether to output intermidate results/progress bar in the console
#' @return a list containing the initial changepoint estimates before the testing refinement, the intermediate estimates after the testing refinement, the intermediate estimates after the midpoint refinement and the estimates after the final refinement.
#' @description From the model y_t = x_t beta_t + eps_t for 1<=t<=z with beta_t = beta^i for z_(i-1) < t <= z_i, for 1 <= i <= nu + 1, where x_t are p-dimensional design vectors for 1<=t<=n, nu is the number of changepoints and z_i is the i-th changepoint with the convention z_0 = 0 and z_(nu+1) = n.  We estimate the changepoint z where the change of regression coefficient takes place.
#' @references Gao, F. and Wang, T. (2022) Sparse change detection in high-dimensional regression.
#' @export
#' @examples
#' # problem parameters
#' n <- 240 # total sample size
...
#' k <- c(3,10) # sparsity of the difference vector of the regression coefficients o
#' k <- c(3,10) # sparsity of the difference vector of the regression coefficients o
#' rho <- 1.5 # difference in l_2 norm of the two regression coefficients
#' # generate the design matrix and response vector of the linear regression sample with multiple changepoints
#' ret <- simulateMultiCP(n,tau,p,k,rho,sigma=1)
#' X <- ret$X; Y <- ret$Y
#' not_result <- not_cpreg(X,Y, Thresh=10)
#' # or  not_result <- cpreg(X,Y,Thresh=NULL,no_intervals=200,burnIn=0.05,cpreg_method='proj')
